...
class InviteRepository(
    ...
    private val BASE_SELECT = "SELECT i.id, i.group_id, g.name AS group_name, g.color AS group_color, " +
            "i.inviter_username, i.invitee_username, i.inviter_email, i.invitee_email, i.status " +
    ...
    private fun ResultSet.toInvite(): Invite = Invite(
        ...
        inviterEmail = getString("inviter_email"),
        inviteeEmail = getString("invitee_email"),
        status = InviteStatus.fromCode(getInt("status")),
    )

    suspend fun create(groupId: Int, inviterUsername: String, inviterEmail: String, invitee: User): Int =
        dataSource.connection.use { connection ->
            withContext(Dispatchers.IO) {
                val stmt = connection.prepareStatement(
                    "INSERT INTO invites (group_id, inviter_username, invitee_username, status, inviter_email, invitee_email) VALUES (?, ?, ?, ?, ?, ?)",
                    Statement.RETURN_GENERATED_KEYS,
                )
                stmt.setInt(1, groupId)
                stmt.setString(2, inviterUsername.lowercase())
                stmt.setString(3, invitee.username.lowercase())
                stmt.setInt(4, InviteStatus.PENDING.code)
                stmt.setString(5, inviterEmail.lowercase())
                stmt.setString(6, invitee.email.lowercase())
                stmt.executeUpdate()
    ...
    suspend fun incomingPendingFor(email: String): List<Invite> = withContext(Dispatchers.IO) {
        dataSource.connection.use { connection ->
            val stmt = connection.prepareStatement(
                "$BASE_SELECT WHERE i.invitee_email = ? AND i.status = ? ORDER BY i.created_at DESC"
            )
    ...
    suspend fun existsPending(groupId: Int, inviteeEmail: String): Boolean = withContext(Dispatchers.IO) {
        dataSource.connection.use { connection ->
            val stmt = connection.prepareStatement(
                "SELECT 1 FROM invites WHERE group_id = ? AND invitee_email = ? AND status = ?"
            )
            stmt.setInt(1, groupId)
            stmt.setString(2, inviteeEmail.lowercase())
